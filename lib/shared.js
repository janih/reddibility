...
var GR = (function () {
  ...
  var DEFAULTS = {
    // On by default: a fresh install has nothing stored, so emptyState()
    // (and thus every apply path) starts active — Reddit is readable the
    // moment the extension lands. Anyone who ever saved settings has an
    // explicit `enabled` value materialized in storage (load() merges the
    // defaults in, and every save persists the full object), so an opt-out
    // always survives this default.
    enabled: true,
    ...
    // Every Reddit tweak defaults to on — fresh installs get the full
    // cleanup out of the box, and users opt out of individual tweaks they
    // dislike. As with `enabled`, anyone who ever saved settings has the
    // full reddit object materialized in storage, so explicit opt-outs
    // always survive a default flip.
    reddit: {
      enabled: true,
      hideAds: true,
